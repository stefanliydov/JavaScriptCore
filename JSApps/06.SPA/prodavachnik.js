...
let password = $("#formLogin").find("input[name=passwd]");
    ...
    function listReq() {
        ...
        function getList (data) {
            ...
            let container = $("#viewAds");
            let source = $("#template").html();
            let template = Handlebars.compile(source);
            data.forEach(a =>{
                    a.isAuthor = a._acl.creator === localStorage.getItem("author")

            });

            container.html(template({ads: data}));
            let btns = container.find("a");
            for(let i =0;i< btns.length;i++){

                if(i%2===0)
                    $(btns[i]).click(() => {deleteAd($(btns).parent().parent().attr("data-type"))});
                else
                    $(btns[i]).click(() => {editAd($(btns).parent().parent().attr("data-type"))});
